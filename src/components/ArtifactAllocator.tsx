  ...
  return (
    <div>
      <div className="field" style={{ marginBottom: 8, maxWidth: 260 }}>
        <label htmlFor="artifactSetName">Artifact set</label>
      ...
      {setInfo && (
        <p className="subtle" style={{ marginBottom: 8 }}>
          Set bonuses are shown for reference; add any active stat or damage effects in Buffs. <strong>2pc:</strong> {setInfo.effect2Pc} <strong>4pc:</strong> {setInfo.effect4Pc}
        </p>
      )}
      ...
      <p className="subtle" style={{ marginBottom: 6 }}>
        <strong style={{ color: 'var(--text)' }}>{total} of {budget}</strong> distributed rolls assigned
        {remaining >= 0 ? ` · ${remaining} unallocated` : ` · ${-remaining} over budget`}.
        {autoReserveER ? ' Energy Recharge rolls are reserved by the Energy planner first.' : ' Energy Recharge is being assigned manually.'}
      </p>

      {!valid && (
        <div className="error-banner">
          {remaining < 0 && <div>Remove {-remaining} manually assigned non-ER roll(s) to make room for the required Energy Recharge.</div>}
      ...
      <div className="table-scroll" style={{ marginBottom: 8 }}>
        <table className="dense">
          <thead><tr><th>Substat</th><th>Fixed</th><th>Distributed</th><th>Cap</th><th>Total rolls</th></tr></thead>
          <tbody>
            {SUBSTAT_TYPES.map((type) => {
              const cap = perSubstatCap(artifacts, type);
              const dist = artifacts.distributed[type];
              const isAutoER = type === 'er' && autoReserveER;
              return (
                <tr key={type}>
                  <td>{SUBSTAT_LABELS[type]} {isAutoER ? <span className="subtle">(reserved by Energy)</span> : null}</td>
                  <td className="num">2</td>
                  <td>
                    {isAutoER
                      ? <span className="count">{dist} reserved</span>
                      : (
                        <div className="stepper">
                          <button type="button" onClick={() => setDistributed(type, dist - 1)} disabled={dist <= 0}>-</button>
                          <span className="count">{dist}</span>
                          <button type="button" onClick={() => setDistributed(type, dist + 1)} disabled={dist >= cap || total >= budget}>+</button>
                        </div>
                      )}
                  </td>
